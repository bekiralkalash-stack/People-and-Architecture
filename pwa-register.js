...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  if(top&&!document.querySelector('[data-visitor-counter]')){
    ...
    fetch('https://counterapi.com/api/peopleandarchitecture.netlify.app/view/platform-visitors?startNumber=1248&unique=true')
      .then(response=>response.ok?response.json():Promise.reject())
      .then(data=>{if(Number.isFinite(data.value))number.textContent=new Intl.NumberFormat('ar').format(data.value)})
