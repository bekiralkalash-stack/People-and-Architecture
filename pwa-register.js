...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  if(top&&!document.querySelector('[data-visitor-counter]')){
    ...
    fetch('https://api.countapi.xyz/hit/architecture-people-platform-2026/visitors', {cache:'no-store'})
      .then(response=>response.ok?response.json():Promise.reject())
      .then(data=>{const count=1248+Number(data.value||0);if(Number.isFinite(count))number.textContent=new Intl.NumberFormat('ar').format(count)})
